...
-- Variáveis de estado para o Aimbot
library.flags["aimbot_enabled"] = false -- Flag principal que seu script Aimbot deve verificar
library.flags["aimbot_key"] = Enum.KeyCode.E -- Keybind padrão
library.flags["aimbot_mode"] = "Hold" -- Modo padrão
library.flags["aimbot_always_active"] = false -- Toggle para o modo Always
...
cursor.Data = game:HttpGet("https://raw.githubusercontent.com/contaloser1802-stack/cbro-script/main/cursor.png")
...
    end
    ...
    -- Lógica de InputEnded para o modo 'Hold' do Aimbot
    local keybind = library.flags["aimbot_key"]
    local mode = library.flags["aimbot_mode"]
    
    -- Garante que o Aimbot desative quando a tecla for solta no modo 'Hold'
    if key.KeyCode == keybind or key.UserInputType == keybind then
        if mode == "Hold" and library.flags["aimbot_enabled"] then
            isHolding = false
            library.flags["aimbot_enabled"] = false
        end
    end
end)
...
            local nextKey = false -- Renomeado 'next' para 'nextKey' para evitar conflito
...
                if tostring(val) ~= "Enum.KeyCode.Unknown" then
...
                local key_val = key.KeyCode == Enum.KeyCode.Unknown and key.UserInputType or key.KeyCode
                
                if nextKey then
                    if not table.find(library.blacklisted,key_val) then
                        nextKey = false
                        updateValue(key_val)
                    end
                end
                
                -- Se a tecla pressionada for a tecla do bind e o menu estiver fechado
                if not nextKey and (key_val == library.flags[args.flag] or key_val.Name == library.flags[args.flag].Name) and args.callback and not menu.Enabled then
                    args.callback()
                end
            end)
...
-- Lógica para Aimbot Bind/Mode (Integrada no addTab e addKeybind)

local combatTab = library:addTab("Combat")
local aimbotGroup = combatTab:createGroup(0) -- Grupo para o Aimbot na coluna da esquerda

aimbotGroup:addList({
    text = "Aimbot Mode",
    flag = "aimbot_mode",
    values = {"Always", "Toggle", "Hold"},
    value = library.flags["aimbot_mode"],
    callback = function(mode)
        -- Resetar o estado ao mudar o modo para evitar comportamento inesperado
        isToggled = false
        isHolding = false
        library.flags["aimbot_enabled"] = false -- Desliga por segurança
        
        if mode == "Always" then
            library.flags["aimbot_enabled"] = library.flags["aimbot_always_active"] -- Se for Always, usa o toggle
        end
    end
})

aimbotGroup:addToggle({
    text = "Aimbot Active (Always)",
    flag = "aimbot_always_active",
    callback = function(state)
        if library.flags["aimbot_mode"] == "Always" then
            library.flags["aimbot_enabled"] = state
        end
    end
})
...
aimbotGroup:addKeybind({
    text = "Aimbot Keybind",
    flag = "aimbot_key", 
    key = library.flags["aimbot_key"],
    callback = function()
        -- Esta função é chamada quando a tecla do Keybind é pressionada (InputBegan)
        local mode = library.flags["aimbot_mode"]
        
        if mode == "Toggle" then
            isToggled = not isToggled
            library.flags["aimbot_enabled"] = isToggled
        elseif mode == "Hold" then
            isHolding = true
            library.flags["aimbot_enabled"] = true
        end
        
        -- O modo 'Always' é controlado pelo Toggle do Aimbot
    end
})


-- Exemplo de como você usaria a flag "aimbot_enabled"
runService.RenderStepped:Connect(function()
    if library.flags["aimbot_enabled"] then
        -- SEU CÓDIGO AIMBOT ENTRA AQUI!
        -- O aimbot só executa a lógica de mira/tiro se esta flag for 'true'
    end
end)
...
        end
    end
    writefile("astrixsete/"..tostring(game.GameId).."/"..name..".cfg",game:GetService("HttpService"):JSONEncode(jig))
...
    if not isfile("astrixsete/"..tostring(game.GameId).."/"..name..".cfg") then
        library:notify("Config file not found!")
        return
    end
    local config = game:GetService("HttpService"):JSONDecode(readfile("astrixsete/"..tostring(game.GameId).."/"..name..".cfg"))
