...
        -- COR DE FUNDO DO GROUPBOX MAIS ESCURA
        groupbox.BackgroundColor3 = Color3.fromRGB(40,40,40)
...
            box.Parent = checkbox
            -- COR DE FUNDO DOS CONTROLES E BORDAS MAIS SUAVES (CLEAN)
            box.BackgroundColor3 = Color3.fromRGB(30,30,30)
            box.BorderColor3 = Color3.fromRGB(80,80,80)
            box.Size = UDim2.new(0,18,0,18)
            
            -- ARREDONDA TOGGLE BOX
...
                state = newState
                -- COR DE FUNDO DOS CONTROLES MAIS ESCURA
                box.BackgroundColor3 = state and library.libColor or Color3.fromRGB(30,30,30)
...
            slider_2.BorderColor3 = Color3.fromRGB(30,30,30)
...
            -- ARREDONDA SLIDER
            Instance.new("UICorner", slider_2).CornerRadius = UDim.new(0, 4)
...
            fill.ZIndex = 0
            
            -- ARREDONDA A BARRA DE PREENCHIMENTO DO SLIDER
...
            valueText.Text = "0"
            -- COR DO TEXTO DE VALOR INATIVO MAIS ESCURA
            valueText.TextColor3 = Color3.fromRGB(100,100,100)
...
                    -- COR DE FUNDO DOS ITENS DA LISTA MAIS ESCURA
                    valueButton.BackgroundColor3 = Color3.fromRGB(30,30,30)
...
            local function colorpicker(a,b,c)
                local d=Color3.fromHSV(a,b,c);
...
                library.flags[args.flag] = colorpicker(h,s,v)
                button.BackgroundColor3 = colorpicker(h,s,v)
                if args.callback then
                    args.callback(colorpicker(h,s,v))
                end
            end
            local hueScrolling = false
            local satScrolling = false
            local hueEnter = false
            local satEnter = false
            local function updateHue()
                if hueScrolling or library.scrolling or library.colorpicking then return end
                while userInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and menu.Enabled do runService.RenderStepped:Wait()
                    library.scrolling = true
                    library.colorpicking = true
                    hueScrolling = true
                    local value = 1 - ((mouse.Y - hue.AbsolutePosition.Y) / hue.AbsoluteSize.Y)
                    if value < 0 then value = 0 end
                    if value > 1 then value = 1 end
                    h = value
                    picker.BackgroundColor3 = colorpicker(h,1,1)
                    updateValue()
                end
                hueScrolling = false
                library.scrolling = false
                library.colorpicking = false
            end
            local function updateSat()
                if satScrolling or library.scrolling or library.colorpicking then return end
                while userInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and menu.Enabled do runService.RenderStepped:Wait()
                    library.scrolling = true
                    library.colorpicking = true
                    satScrolling = true
                    local value = ((mouse.X - picker.AbsolutePosition.X) / picker.AbsoluteSize.X)
                    local value2 = 1 - ((mouse.Y - picker.AbsolutePosition.Y) / picker.AbsoluteSize.Y)
                    if value < 0 then value = 0 end
                    if value > 1 then value = 1 end
                    if value2 < 0 then value2 = 0 end
                    if value2 > 1 then value2 = 1 end
                    s = value
                    v = value2
                    updateValue()
                end
...
                if satScrolling or satEnter then return end
                satEnter = true
                while satEnter do runService.RenderStepped:Wait()
                    updateSat()
                end
            end)
...
                if hueScrolling or hueEnter then return end
                hueEnter = true
                while hueEnter do runService.RenderStepped:Wait()
                    updateHue()
                end
            end)
...
                        local hue,sat,val = button.BackgroundColor3:ToHSV()
                        h = hue
                        s = sat
                        v = val
                        picker.BackgroundColor3 = colorpicker(h,1,1)
...
            library.options[args.flag] = {type = "colorpicker",changeState = function(val) local a,b,c = val:ToHSV() h=a s=b v=c updateValue() end,skipflag = args.skipflag,oldargs = args}
            local a,b,c = library.flags[args.flag]:ToHSV()
            h=a s=b v=c
            updateValue()
        end
        function group:addLabel(args)
            if not args.text then return warn("⚠️ incorrect arguments ⚠️") end
            groupbox.Size += UDim2.new(0,0,0,18)
...
-- CORREÇÃO PRINCIPAL: RETORNA OS OBJETOS DA BIBLIOTECA
-- Se este script for executado via loadstring(...)(), ele retornará os três valores.
