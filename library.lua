...
-- Observação: O objeto 'menu' com ID 7142010382 deve existir no seu Roblox.
...
                end
            else
                -- CORREÇÃO DE CÓDIGO AQUI: O bloco else/if/else estava incompleto.
                if v.type == "toggle" then
                    v.changeState(false) -- Opções não salvas são definidas como falso
                end
            end
        end)end) -- Fechando o pcall e o spawn
    end
    library:notify("Successfully loaded config "..name..".cfg!")
end

function library:refreshConfigs()
    -- Função vazia para simular a atualização da lista de configs
end

--- FIM DO CÓDIGO DA BIBLIOTECA. COMEÇO DO SEU MENU DE USO ---

## ⚙️ Exemplo de Uso do Menu (Para Testar a Biblioteca)

-- 1. CONFIGURAÇÃO BÁSICA E ABA PRINCIPAL (COMBATE)
local combatTab = library:addTab("Combate")
local combatGroupLeft, _ = combatTab:createGroup(0) -- Coluna da Esquerda (0)

combatGroupLeft:addToggle({
    text = "Aimbot Ativo",
    flag = "aimbot_enabled",
...
combatGroupLeft:addSlider({
    ...
    callback = function(value)
        print("Alcance (range): " .. value)
    end
})

combatGroupLeft:addDivider() -- Linha divisória
combatGroupLeft:addKeybind({
    text = "Atalho do Aimbot",
    flag = "aimbot_key",
    key = Enum.UserInputType.MouseButton2, -- Botão direito do mouse
    callback = function()
        library:notify("Keybind de Aimbot pressionado!")
    end
})

-- 2. ABA DE VISUAIS (VISUALS)
local visualsTab = library:addTab("Visuais")
local visualsGroupLeft, _ = visualsTab:createGroup(0)

visualsGroupLeft:addToggle({
    text = "Wallhack (ESP)",
    flag = "esp_active",
    callback = function(state)
...
visualsGroupLeft:addColorpicker({
    text = "Cor do Traço",
    flag = "esp_color",
    color = Color3.fromRGB(255, 0, 0), -- Cor vermelha inicial
    callback = function(color)
        print("Nova cor do ESP: " .. color:ToHex())
    end
})

-- 3. ABA DE MISCELÂNEA (MISC)
local miscTab = library:addTab("Misc")
local miscGroupLeft, _ = miscTab:createGroup(0)

miscGroupLeft:addList({
    text = "Seletor de Arma",
    flag = "selected_weapon",
    values = {"Faca", "Pistola", "Rifle", "Sniper"},
    callback = function(value)
        print("Arma Selecionada: " .. value)
    end
})

miscGroupLeft:addButton({
    text = "Teleportar (Exemplo)",
    callback = function()
        library:notify("Ação de Teleporte Executada!")
    end
})

-- 4. ABA DE CONFIGURAÇÃO (LOAD/SAVE)
local configTab = library:addTab("Config")
local configGroupLeft, _ = configTab:createGroup(0)

configGroupLeft:addTextbox({
    text = "Nome do Perfil",
    flag = "config_name",
})

configGroupLeft:addDivider()

configGroupLeft:addList({
    text = "Carregar Perfil",
    flag = "selected_config",
    values = {"Default", "MeuPerfil1", "MeuPerfil2"},
})

configGroupLeft:addButton({
...
-- Inicializa o menu mostrando a primeira aba
for i, v in next, library.tabs do
    v.Visible = i == 1
    break
end
